...
export const errors = {
  ...
  timeout: (message: string = '', details?: any) => {
    console.error('API Request Timeout:', message, details);
  ...
  methodNotAllowed: (message: string = 'Method Not Allowed', details?: any): Response => {
    let headers = {};
    
    // If details includes supportedMethods, add Allow header
    if (details?.supportedMethods && Array.isArray(details.supportedMethods)) {
        headers = {
            'Allow': details.supportedMethods.join(', ')
        };
    }
