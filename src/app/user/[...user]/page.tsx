...
import { Repository, RepoArray } from "@/types/repos";
  ...
  return (
    <div className="space-y-4">
      {[...Array(REPO_COUNT)].map((_, i) => (
        <div
          key={i}
          className="animate-pulse space-y-4 rounded-lg border border-github-border p-4 bg-github-dark-secondary"
        >
...
async function fetchUserData(username: string) {
  const [userRes, reposRes, starredRes] = await Promise.all([
    ...
    fetch(
      `https://api.github.com/search/repositories?q=user:${username}&sort=stars&order=desc&per_page=${REPO_COUNT}`,
    ),
    fetch(
      `https://api.github.com/users/${username}/starred?per_page=${REPO_COUNT}`,
    ),
  ]);
      ...
      <div className="card mb-8 p-6 bg-github-dark rounded-lg shadow-lg">
        <div className="flex flex-col md:flex-row md:items-start gap-6">
          <motion.div
            ...
            className="relative flex h-24 w-24 shrink-0 overflow-hidden rounded-full"
          >
            <img
          ...
          <div className="space-y-3">
            ...
            {user.bio && (
              <p className="text-sm max-w-3xl text-white">{user.bio}</p>
            )}
