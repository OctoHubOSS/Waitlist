...
const ModernFileExplorerContext = createContext<FileExplorerContextType | null>(
  null,
);
...
export function useModernFileExplorer(): FileExplorerContextType {
  ...
  if (!context) {
    throw new Error(
      "useModernFileExplorer must be used within a ModernFileExplorerProvider",
    );
  }
  return context;
}
...
export function ModernFileExplorerProvider({
  children,
  value,
}: ModernFileExplorerProviderProps) {
