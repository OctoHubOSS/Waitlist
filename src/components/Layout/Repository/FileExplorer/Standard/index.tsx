...
import { findReadmeFile } from "../../Shared/utils/filetype";
...
export default function StandardFileBrowser({
  ...
  const { data: fileData, isLoading: contentsLoading } = useRepositoryFiles(
    owner,
    repo,
    currentPath,
    selectedBranch,
  ...
  useEffect(() => {
    if (branchesData) {
      ...
      if (
        !defaultBranch ||
        !branchesData.some((b: Branch) => b.name === selectedBranch)
      ) {
        const mainBranch = branchesData.find((b: Branch) =>
          ["main", "master", "develop", "development"].includes(
            b.name.toLowerCase(),
          ),
        );
        setSelectedBranch(
          mainBranch ? mainBranch.name : branchesData[0]?.name || "main",
        );
      }
    }
