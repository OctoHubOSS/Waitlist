...
export default function RepositoryHeader({
  ...
  const hasMoreTopics =
    repository.topics && repository.topics.length > MAX_VISIBLE_TOPICS;

  // Get visible topics based on current state
  const visibleTopics = repository.topics
    ? showAllTopics
      ? repository.topics
      : repository.topics.slice(0, MAX_VISIBLE_TOPICS)
    : [];
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-start gap-8">
      ...
      <div className="space-y-4 flex-1">
        ...
        >
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl md:text-4xl font-bold text-github-text">
                {repository.name}
              </h1>
          ...
          <motion.a
            ...
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="px-4 py-2 btn btn-primary text-white font-semibold rounded-lg transition-all flex items-center"
          >
        ...
        <motion.p
          variants={itemVariants}
          className="text-md max-w-3xl text-github-text"
        >
              ...
              {hasMoreTopics && (
                ...
                >
                  {showAllTopics ? (
                    ...
                    <>
                      <span>
                        +{repository.topics.length - MAX_VISIBLE_TOPICS} more
                      </span>
