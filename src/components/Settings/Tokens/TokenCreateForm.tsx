...
export default function TokenCreateForm({
  ...
  const handleTypeChange = (newType: "basic" | "advanced") => {
    ...
    if (newType === "basic") {
      setScopes((prevScopes) =>
        prevScopes.filter(
          (scope) => scope.includes(":read") || scope === "search:read",
        ),
      );
  ...
  return (
    <form onSubmit={handleSubmit} className="card card-hover p-6 space-y-8">
      <div className="space-y-6">
        <div>
          <label
            htmlFor="token-name"
            className="block text-sm font-medium text-github-text-secondary mb-2"
          >
          ...
          <p className="mt-1 text-xs text-github-text-secondary">
            Choose a descriptive name that helps you remember where this token
            is used.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-github-text-secondary">
            For security reasons, all tokens expire. You can generate a new
        ...
        <div>
            ...
            >
              ...
              {availableScopeGroups.map((group) => (
                <option
                  ...
                  disabled={
                    type === "basic" &&
                    !["BASIC_READ", "FULL_READ"].includes(group.key)
                  }
                >
                  {group.name}
                  {type === "basic" &&
          ...
          {customScopes && (
            <div className="bg-github-dark-secondary/40 border border-github-border rounded-md p-4 space-y-4">
              {Object.entries(allScopes).map(([category, categoryScopes]) => (
                <div key={category}>
                  <h4 className="text-sm font-medium text-github-link mb-2">
                    {category}
                  </h4>
          ...
          <div className="mt-2 flex items-center">
            <span className="text-xs text-github-text-secondary mr-2">
              Selected scopes:
            </span>
